...
log.warn(servicesToEmailThem)
 ...
 if (emailBodyId in['Announcement_Template_02','Announcement_Template_01']){
    if (issue.status.name == "Testing") {
        sendEmailSingleOutageToTestOrReview(servicesToEmailThem,issue, [issue.reporter.emailAddress], 'reporter')
        ...
        sendEmailSingleOutageToTestOrReview(servicesToEmailThem, issue, reviewerEmailList, 'reviewer')
    } else {
...
/**
 * Send announcement Email with single outage to test/review team
 */
def sendEmailSingleOutageToTestOrReview(def serviceList,def issue ,def sendTo , def senderType){
...
/**
 * Send announcement Email with multiple outage to test/review team
 */
...
/**
 * Send announcement Email with single outage to client (external)
 */
...
/**
 * Send announcement Email with single outage to client (external & internal)
 */
...
/**
 * Send announcement Email with multiple outage to client (external)
 */
...
/**
 * Send announcement Email with single outage to client (internal only)
 */
...
//        log.warn(internalEmailList)
    ...
    if(templateType = 'Announcement_Template_01') {
        def sendJiraEmail = SendJiraEmail.sendEmailToUsersAnnouncement('no_reply@elm.sa', '', customerEmailList.unique(true).join(','), subject, plannedServiceOutageAnnouncementInHtmlFormat(
        ...
        def sendJiraEmail = SendJiraEmail.sendEmailToUsersAnnouncement('no_reply@elm.sa', '', customerEmailList.unique(true).join(','), subject, plannedServicesInstabilityAnnouncementInHtmlFormat(
                serviceName_OutageStart:serviceName_OutageStart,
                serviceName_OutageEnd:serviceName_OutageEnd,
                formatDuration:formatDuration,
                serviceList))
