...
/**
 * send support plan in pdf format to the client
 */

/**
 * get the required customFields for the service name
 */
...
def sendEmailSingleOutageToClient(def serviceList,def issue, def senderType) {
    ...
    def emailList = CommonUtil.getInsightCFieldObject(issue, """objectType = "Stakholder Data" AND Status = Active AND "Service" = "${serviceList[0]}" AND SupportPlan = Yes """, 1)
        ...
        def sendJiraEmail = SendJiraEmail.sendEmailToUsersAnnouncementWithAttachment('no_reply@elm.sa', "",customerEmailList.join(','), subject, plannedSupportAnnouncementInHtmlFormat(serviceList), filePath, 'SupportPlan')
    }
...
/**
 * send a well formatted email to the internal clients
 */
