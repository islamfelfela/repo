...
/**
 * a condition that checks if the issue is a CDX issue with operations team
 */

def mainMethod() {
//    ApplicationUser botUser = ComponentAccessor.getUserManager().getUserByName("bot")
    def jjmProjectNatureObject = CommonUtil.getCustomFieldObject(13708)?.name?.toString() // jenmkins mapping project nature ops or without ops
    def jjmProjectKeyObject = CommonUtil.getCustomFieldObject(13608)?.name?.toString()  // Project key in jenkins mapping
    def changeProjectKey = CommonUtil.getProjectObjByIssue(issue)?.key?.toString()  // Project Key of the change
    def changeJenkinsBranch = CommonUtil.getCustomFieldValue(issue, 13709)?.toString()?.toLowerCase()// Jenkin branch from the change
...
//check for selected jenkins branch and project in mapping, if not mapping found it means its non cdx projects and it should enable the send to cab transition.
    if (mappingResultsWithOps.isEmpty() && mappingResultsWithoutOps.isEmpty()) {
        ...
        passesCondition=!mappingResultsWithOps.isEmpty()
    }
}
mainMethod()